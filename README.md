...
If the pump is on (`vfd` > 0) and pressure stays below `PRESSURE_LOW_PSI` for `PRESSURE_LOW_ALARM_DURATION_MS`, or above `PRESSURE_HIGH_PSI` for `PRESSURE_HIGH_ALARM_DURATION_MS`, or if battery drops below `PRESSURE_BATTERY_LOW_PCT`, the station POSTs to `VFD_ALERT_URL` with the configured summary strings (same JSON shape as the VFD fault alert). A low-pressure alarm also locks the VFD off. Turn **Pump safety** off on the station web UI (or `POST /pressure_safety` with `enabled=0`) to clear that lockout and skip low/high pressure involvement without reflashing; the choice is stored in NVS. Zero a drifted sensor from the station or solenoid UI (**Zero now**, or `POST /pressure_offset` with `action=zero`) so idle atmosphere reads 0 psi.

Raw values use **signed int16 big-endian tenths of a psi** by default (`BLE_PRESSURE_SCALE_TENTHS`, `BLE_PRESSURE_BIG_ENDIAN`): `psi = raw / 10` as two's-complement, MSB first (e.g. nRF `03-A8` → `0x03A8` → 94 psi, `FF-F4` → `0xFFF4` → −1.2 psi). Negative readings are not clamped near zero, so a drifted baseline (for example −47 psi idle) is reported instead of sticking at −20. Set `BLE_PRESSURE_BIG_ENDIAN` to `0` for LSB-first sensors, or `BLE_PRESSURE_SCALE_TENTHS` to `0` for legacy two-point linear scaling.
